...
I  = imread(fileName);
I = colorize(I);
bwImage = blackwhite(I);
edgeMap = edge(bwImage);
%bwImage = 1-I(:,:,1) / 255;
...
fprintf('\nImproved _ with gradient optimized_: Starting on %s (N_theta = %d, N_rho = %d)\n', fileName, N_theta,N_rho);
...
[gx,gy] = gradient(double(bwImage),.2,.2);
thetaMiddleInds = zeros(length(xind), 1);
for k = 1:length(xind)
    x = xind(k);
    y = yind(k);
    theta = mod(atan2(gy(y,x), gx(y,x)), pi);   
    thetaInd = 1+min(max(int32(theta / (pi / N_theta)), 0), N_theta-1);
    thetaMiddleInds(k) = thetaInd; 
end
...
for z = 1:edgeCount
    ...
    for k = 1:length(xind)
        ...
        xAdj = x - halfX;
        yAdj = y - halfY;
        dThetaInd = int32(N_theta/10);
        for thetaInd = 0:N_theta
        %for i = -dThetaInd:dThetaInd
         %   thetaInd = 1+mod((thetaMiddleInds(k)-1) + i, N_theta);
            
            if thetaInd == 0
                rhoInd = 0;
            else
                rho = xAdj*coss(thetaInd) + yAdj*sins(thetaInd);
                rhoInd = 1 + min(max(int32(((rho + R) * N_rho) / R + 1), 0), 2*N_rho-1);
            end
    ...
    imagesc(M); colormap(hot)
    print(fig,'-dpng',sprintf('../../report/gradient_voting_space_%2d.png', z));
